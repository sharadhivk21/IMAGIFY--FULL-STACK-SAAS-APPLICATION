...
const Login = () => {
    ...
    //need to pass these to the api to create an account
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            if (state === 'Login') {
                ...
                if (data.success) { //successful login condition
                    //get token from res and set token in appcontext to connect to frontend
                    setToken(data.token)
                    setUser(data.user)

                    //storing token in browser local storage
                    localStorage.setItem('token', data.token)

                    //collapse login form after successful login
                    setShowLogin(false)
                } else {
                ...
                if (data.success) { //successful registration condition
                    //get token from res and set token in appcontext to connect to frontend
                    setToken(data.token)
                    setUser(data.user)
    ...
    return (
        <div className="fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center">
            <motion.form onSubmit={onSubmitHandler}
                ...
                {/*  user name stored in its state variable*/}
                {state !== 'Login' && (
                ...
                {/* user email when entered will be stores in its respective state variable*/}
                ...
                {/* password  stored in its state variable*/}
                <div className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4">
